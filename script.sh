...
telegram_config_complete() {
  ...
  grep -q "^appapiid[[:space:]]*=" "$BOT_CONFIG_PATH" || return 1
  grep -q "^appapihash[[:space:]]*=" "$BOT_CONFIG_PATH" || return 1
}
...
start_telegram_installer() {
  ...
  if [ -x "$INSTALLER_SERVICE_PATH" ]; then
    ...
    echo "Telegram-настройки не заполнены. Откройте web-интерфейс на порту $(config_get 'browser_port' '8080') и заполните token, username, app api id/hash."
  else
...
BOT_RUNTIME_MODULES="web_form_template.py web_form_blocks.py web_pool_form_blocks.py web_http_common.py web_get_actions.py web_post_actions.py web_command_state.py unblock_lists.py proxy_key_store.py proxy_protocols.py proxy_config_builder.py proxy_status.py installer_common.py"
