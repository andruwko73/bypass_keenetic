...
from service_catalog import YOUTUBE_UNBLOCK_ENTRIES
...
def route_contains_youtube(filename):
    ...
    try:
        ...
        return False
    return any(entry_matches(entry, youtube_entry) for entry in entries for youtube_entry in YOUTUBE_UNBLOCK_ENTRIES)
