...
def _router_health():
    return {
        "memory_text": "Память: доступно 148 МБ, занято 338 МБ из 486 МБ (70%)",
        "note": "Нагрузка CPU: 2%\nПрограмма использует 42 МБ ОЗУ; Flash-носитель: занято 384 из 1024 МБ (38%)",
        ...
        "used_percent": 70,
        "memory_state": "economy",
        "memory_tone": "info",
        "memory_reason": "Доступно 148 МБ; ресурсоёмкие фоновые проверки работают в экономичном режиме",
...
class FixtureHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/route_move_status":
            self._json({"running": False, "stage": "", "elapsed_seconds": 0})
            return
