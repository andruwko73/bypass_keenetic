...
def update_key_probe_cache_entry(
    ...
    *,
    ...
    if yt_ok is not None:
        ...
        if entry.get('yt_ok') is not value:
            entry['yt_ok'] = value
            changed = True
...
class KeyProbeBatchRecorder:
    ...
    def record(self, proto, key_value, tg_ok=None, yt_ok=None, custom=None):
        should_flush = False
        with self._lock:
            self._pending.append((proto, key_value, tg_ok, yt_ok, custom, time.time()))
    ...
    def flush(self):
        ...
        with _cache_lock:
            ...
            changed = False
            for proto, key_value, tg_ok, yt_ok, custom, ts in pending:
...
def record_key_probe(proto, key_value, tg_ok=None, yt_ok=None, custom=None, *, min_write_interval=KEY_PROBE_MIN_WRITE_INTERVAL):
    with _cache_lock:
        cache = load_key_probe_cache()
