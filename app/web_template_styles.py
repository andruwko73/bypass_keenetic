def render_web_styles(TELEGRAM_SVG_B64=''):
    return '''        :root{
        ...
        [data-theme="light"]{
            ...
            --warn-bg:#e8edf3;
            --warn-border:#b8c6d4;
            --shadow:0 18px 40px rgba(43, 55, 71, 0.14);
        }
        *{box-sizing:border-box;}
        ...
        [data-theme="light"] body{
            background:
                radial-gradient(circle at top left, rgba(104,119,138,.12), transparent 34%),
                radial-gradient(circle at top right, rgba(31,122,106,.08), transparent 28%),
        ...
        [data-theme="light"] .hero{background:linear-gradient(140deg, rgba(229,234,240,.98), rgba(212,220,229,.92));}
        ...
        .theme-toggle:hover{filter:none;transform:none;background:rgba(35,98,104,.44);border-color:rgba(96,214,205,.62);}
                ...
                .hero-popover{position:absolute;top:54px;right:0;min-width:260px;padding:14px;border:1px solid var(--border);border-radius:18px;background:linear-gradient(180deg, rgba(23,30,40,.98), rgba(32,42,56,.96));box-shadow:var(--shadow);z-index:320;}
                [data-theme="light"] .hero-popover{background:linear-gradient(180deg, rgba(229,234,240,.98), rgba(216,223,232,.96));}
                .hidden{display:none;}
        ...
        [data-theme="light"] .panel{background:linear-gradient(180deg, rgba(229,234,240,.96), rgba(216,223,232,.94));}
        ...
        .pool-apply-btn{width:100%;min-width:0;padding:4px 0;border:none;background:transparent;box-shadow:none;color:var(--text);font-size:12px;font-weight:700;text-align:left;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:flex;align-items:center;justify-content:flex-start;gap:6px;}
        ...
        .pool-import-form textarea{min-width:0;width:100%;max-width:100%;min-height:92px;resize:none;overflow:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;}
        .pool-import-form button{justify-self:start;min-width:160px;height:var(--control-height);min-height:var(--control-height);padding:0 12px;white-space:nowrap;}
        ...
        [data-theme="light"] .topbar{background:rgba(231,235,240,.96);}
        ...
        [data-theme="light"] .side-nav{background:rgba(231,235,240,.94);}
        ...
        [data-theme="light"] .view-head{background:rgba(231,235,240,.92);}
        .view-head h2{margin:0 0 6px;font-size:24px;}
        .status-overview-head{display:block;padding:12px 14px;}
        .status-overview-copy{display:flex;align-items:baseline;gap:10px;min-width:0;flex-wrap:wrap;}
        .status-overview-head h2{margin:0 0 4px;font-size:16px;line-height:1.22;font-weight:700;color:var(--text);}
        .status-overview-head .section-subtitle{font-size:12px;line-height:1.35;}
        ...
        .health-meter.warn span{background:linear-gradient(90deg,#d3a557,#f6c892);}
        .health-meter.danger span{background:linear-gradient(90deg,#d46b5f,#ffb7b1);}
        .attention-list{list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px;}
        ...
        [data-theme="light"] .protocol-workspace,[data-theme="light"] .list-workspace{background:linear-gradient(180deg, rgba(231,235,240,.96), rgba(216,223,232,.94));}
        ...
        .subtabs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border:1px solid var(--border);border-radius:8px;overflow:hidden;margin-bottom:14px;}
        ...
        .pool-toolbar{display:grid;grid-template-columns:repeat(3,minmax(150px,200px));align-items:center;justify-content:start;gap:10px;margin-bottom:12px;}
        .pool-toolbar form{display:block;}
        .pool-controls{display:grid;grid-template-columns:minmax(240px,520px) minmax(180px,240px);justify-content:start;gap:8px;margin:0 0 8px;}
        .pool-controls input,.pool-controls select,.pool-sort-button{width:100%;height:var(--control-height);min-height:var(--control-height);}
        .pool-sort-control{position:relative;min-width:0;}
        .pool-sort-button{justify-content:space-between;padding:0 12px;text-align:left;}
        .pool-sort-button::after{content:"⌄";flex:none;margin-left:10px;font-size:14px;line-height:1;}
        .pool-sort-menu{position:absolute;top:calc(100% + 6px);left:0;right:0;z-index:230;display:grid;gap:4px;padding:6px;border:1px solid var(--border);border-radius:8px;background:rgba(16,24,34,.98);box-shadow:0 18px 44px rgba(0,0,0,.34);}
        .pool-sort-menu.hidden{display:none;}
        .pool-sort-option{width:100%;height:32px;min-height:32px;justify-content:flex-start;padding:0 10px;border-radius:6px;border:1px solid transparent;background:transparent;color:var(--text);box-shadow:none;font-size:12px;}
        .pool-sort-option:hover,.pool-sort-option.active{background:rgba(31,122,106,.18);border-color:rgba(78,216,205,.28);color:#96f1eb;}
        .pool-sort-divider{display:block;margin:2px 2px 0;padding:7px 8px 2px;border-top:1px solid rgba(91,124,150,.32);font-size:10px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);}
        .pool-row-hidden{display:none!important;}
        ...
        .pool-mobile-checked{display:none;}
        ...
        body{font-family:"Segoe UI",Arial,Helvetica,sans-serif;background:linear-gradient(145deg,#0c1118 0%,#111821 48%,#0b1016 100%);padding:14px;}
        ...
        *::-webkit-scrollbar-thumb:hover{background:var(--scrollbar-thumb-hover);}
        [data-theme="light"] body{background:linear-gradient(145deg,#e6ebf0 0%,#d8dfe7 100%);}
        ...
        .topbar,.side-nav,.view-head,.panel,.protocol-workspace,.list-workspace,.confirm-card,.event-history-drawer{border-radius:10px;background:rgba(17,25,35,.88);border-color:rgba(91,124,150,.34);box-shadow:0 14px 34px rgba(0,0,0,.22);backdrop-filter:blur(10px);}
        .topbar{position:relative;z-index:260;top:0;padding:8px 10px;margin-bottom:8px;justify-content:stretch;min-height:0;overflow:visible;}
        ...
        .topbar-actions{width:100%;display:grid;grid-template-columns:minmax(420px,.85fr) minmax(520px,1.4fr) minmax(176px,220px) auto;align-items:center;justify-content:stretch;gap:8px;min-height:0;max-width:100%;}
        .topbar-actions.topbar-actions-web-only{grid-template-columns:minmax(0,1fr) max-content auto;}
        .theme-control{justify-self:end;width:100%;}
        ...
        .mode-control #mode-picker{right:0;}
        .theme-control .theme-picker{right:0;}
        .app-caption{display:block;min-width:0;color:#eef7ff;white-space:normal;overflow-wrap:anywhere;word-break:break-word;}
        .app-caption strong{display:block;max-width:none;font-size:15px;font-weight:800;line-height:1.18;letter-spacing:0;white-space:normal;overflow:visible;text-overflow:clip;overflow-wrap:normal;word-break:normal;}
        ...
        [data-theme="light"] .version-badge{background:rgba(231,235,240,.84);}
        ...
        .topbar-status-info{border-color:rgba(91,124,150,.42);background:rgba(17,25,35,.76);}
        .topbar-status-warn{border-color:rgba(211,165,87,.42);background:rgba(78,52,25,.32);}
        .topbar-status-danger{border-color:rgba(212,107,95,.46);background:rgba(84,35,39,.34);}
        ...
        .inline-page-title{display:flex;align-items:baseline;gap:8px;min-width:0;margin:0 0 4px;color:var(--text);font-size:13px;font-weight:800;line-height:1.22;letter-spacing:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
        .view-head h2.inline-page-title,.workspace-head h2.inline-page-title,.protocol-lazy-placeholder h2.inline-page-title{margin:0 0 4px;font-size:13px;font-weight:800;line-height:1.22;}
        .protocol-workspace .workspace-head h2.inline-page-title{font-size:13px;}
        .inline-page-title .title-kicker{flex:none;color:#d3a557;font-size:inherit;font-weight:inherit;letter-spacing:0;text-transform:none;line-height:inherit;}
        .inline-page-title .title-kicker::after{content:" ·";letter-spacing:0;color:#8fa3b7;}
        .inline-page-title span:last-child{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
        .status-overview-head{padding:10px 12px;}
        .status-overview-copy{gap:9px;}
        .status-overview-head h2{font-size:13px;font-weight:800;}
        .status-head-attention{grid-template-columns:repeat(auto-fit,minmax(210px,1fr));}
        .eyebrow{color:#d3a557;font-size:11px;letter-spacing:.14em;}
        .section-subtitle{font-size:12px;line-height:1.35;}
        ...
        .router-health-card .card-icon{font-size:10px;font-weight:800;letter-spacing:0;}
        .card-icon img{width:17px!important;height:17px!important;}
        .status-label{margin:0 0 4px;color:#edf5fb;font-size:13px;font-weight:800;letter-spacing:0;text-transform:none;}
        ...
        .attention-list{grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:7px;}
        .attention-item{padding:6px 8px;}
        .attention-item strong,.attention-item span:last-child{font-size:11.5px;}
        ...
        .service-panel h3{font-size:13px;font-weight:800;line-height:1.22;margin-bottom:8px;}
        ...
        .event-history-panel{margin-top:8px;display:grid;gap:5px;}
        .event-history-list{list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:4px;}
        .event-history-item{display:grid;grid-template-columns:58px minmax(0,1fr);gap:6px;align-items:start;padding:5px 6px;border-radius:7px;border:1px solid rgba(91,124,150,.28);background:rgba(255,255,255,.025);line-height:1.15;}
        .event-time{font-size:9.5px;color:#9fb0c8;white-space:nowrap;padding-top:1px;}
        .event-main{display:grid;gap:1px;min-width:0;}
        .event-title-row{display:flex;align-items:baseline;gap:5px;min-width:0;}
        .event-main strong{flex:none;max-width:42%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10.5px;color:#edf5fb;}
        .event-main small{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:9.5px;color:#9fb0c8;}
        ...
        body.event-history-open{overflow:hidden;}
        .event-history-backdrop{position:fixed;inset:0;z-index:720;display:flex;align-items:stretch;justify-content:flex-end;padding:14px;background:rgba(2,6,23,.62);overscroll-behavior:contain;}
        .event-history-backdrop.hidden{display:none;}
        .event-history-drawer{width:min(720px,calc(100vw - 28px));max-height:calc(100dvh - 28px);min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr);padding:14px;border:1px solid var(--border);background:rgba(17,25,35,.96);overflow:hidden;overscroll-behavior:contain;}
        ...
        .event-history-drawer [data-event-history-pane]{min-height:0;overflow:hidden;}
        .event-history-content{display:grid;grid-template-rows:auto minmax(0,1fr);gap:8px;min-height:0;height:100%;overflow:hidden;}
        .event-history-drawer [data-event-history-list]{min-height:0;overflow:hidden;display:grid;}
        ...
        .event-history-drawer .event-history-list{min-height:0;max-height:100%;overflow:auto;padding-right:4px;grid-template-columns:1fr;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;}
        ...
        .protocol-subview[data-subview="pool"].active{min-height:0;grid-template-rows:auto auto auto;align-content:start;}
        .protocol-subview[data-subview="pool"].active .pool-table-wrap{min-height:0;}
        .pool-table-wrap,.list-editor-form textarea,.key-editor-form textarea,textarea{scrollbar-gutter:stable;scrollbar-color:var(--scrollbar-thumb) var(--scrollbar-track);scrollbar-width:thin;}
        ...
        .protocol-workspace .workspace-head h2{margin:0;font-size:19px;font-weight:800;}
        .protocol-workspace .workspace-head .eyebrow{margin-bottom:5px;}
        .key-status-note{margin:3px 0 0;font-size:12px;line-height:1.35;}
        ...
        .pool-toolbar button,.pool-clear-btn{height:var(--control-height);min-height:var(--control-height);padding:0 12px;line-height:1.15;width:100%;}
        .pool-controls{gap:7px;margin-bottom:7px;}
        ...
        @media (min-width: 1024px){
            ...
            .topbar{position:relative;z-index:260;top:0;flex:none;margin-bottom:8px;padding:8px 10px;}
            .topbar-actions{grid-template-columns:minmax(560px,1.25fr) minmax(260px,.75fr) minmax(136px,180px) auto;}
            .app-caption strong,.app-branch{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;overflow-wrap:normal;word-break:normal;}
            .app-caption strong{font-size:15px;line-height:1.1;}
            .app-branch{font-size:11px;line-height:1.15;}
            .section-subtitle{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;overflow-wrap:normal;word-break:normal;}
            ...
            .status-dashboard-no-bot .status-dashboard-column-secondary{grid-template-rows:1fr;}
            .status-card{min-height:0;padding:9px;}
            .router-health-card,.quick-start-card,.active-mode-card{min-height:0;}
            .router-health-card .status-copy{display:grid;grid-template-rows:auto auto auto minmax(0,1fr) auto;min-height:0;}
            .router-health-card #router-health-note{min-height:0;max-height:72px;overflow:auto;padding-right:4px;}
            .router-health-actions{grid-template-columns:1fr;margin-top:8px;}
            ...
            .protocol-subview[data-subview="pool"].active{display:grid;grid-template-rows:auto auto auto;align-content:start;overflow:visible;}
            .pool-table-wrap{max-height:min(48vh,520px);height:auto;min-height:0;}
            ...
            .protocol-subview-key .key-editor-form{display:grid;grid-template-columns:minmax(0,1fr) 180px;grid-template-rows:auto auto;gap:6px 10px;align-items:start;}
            .protocol-subview-key .key-editor-form .field-label{grid-column:1 / -1;margin:0;line-height:1.1;}
            .protocol-subview-key .key-editor-form textarea[data-key-textarea]{grid-column:1;grid-row:2;height:72px;min-height:72px;max-height:none;resize:none;white-space:pre-wrap;overflow:hidden;overflow-wrap:anywhere;word-break:break-all;}
            .protocol-subview-key .key-editor-form .form-actions{grid-column:2;grid-row:2;margin:0;display:grid;align-items:start;}
            ...
            .pool-import-form textarea{grid-column:1;height:112px;min-height:112px;max-height:none;resize:none;overflow:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;}
            .pool-import-form .subscription-hwid-toggle{grid-column:1;align-self:center;}
            .pool-import-form button{grid-column:2;grid-row:3;justify-self:stretch;width:100%;align-self:start;}
        ...
        @media (min-width: 1024px){
            .protocol-workspace.active{height:100%;min-height:0;display:grid;grid-template-rows:auto auto minmax(0,1fr);overflow:hidden;}
            .protocol-workspace.active:has(.protocol-subview[data-subview="pool"].active){height:100%;display:grid;overflow:hidden;}
            .protocol-workspace.active:has(.protocol-subview[data-subview="pool"].active) .protocol-subview[data-subview="pool"].active{min-height:0;display:grid;grid-template-rows:auto auto minmax(0,1fr);align-content:start;overflow:hidden;}
        ...
        @media (min-width: 761px) and (max-width: 1180px){
            ...
            .topbar-actions{grid-template-columns:minmax(360px,.8fr) minmax(360px,1.3fr) minmax(140px,180px) auto;gap:7px;}
            .topbar-actions.topbar-actions-web-only{grid-template-columns:minmax(0,1fr) max-content auto;}
        ...
        @media (min-width: 761px) and (max-width: 920px){
            .topbar-actions{grid-template-columns:minmax(260px,.78fr) minmax(0,1.22fr) minmax(92px,130px) auto;gap:6px;}
            .topbar-actions.topbar-actions-web-only{grid-template-columns:minmax(0,1fr) max-content auto;}
            #mode-toggle-button span:first-child,
            #theme-toggle-button span:first-child{display:none;}
            .api-pill{font-size:10.5px;}
            .mode-toggle,.theme-toggle{padding-left:6px;padding-right:6px;}
        }
        @media (min-width: 1024px) and (max-height: 760px){
            html,body{height:100%;overflow:hidden;}
            .app-shell{height:calc(100vh - 16px);min-height:0;}
            .workspace-layout{align-items:stretch;}
            .app-main,.app-view.active{height:100%;min-height:0;overflow:hidden;}
            .view-head,.segmented,.status-dashboard,.overview-service-grid{margin-bottom:0;}
            .protocol-panels,.list-panels{overflow:hidden;}
            .protocol-workspace.active,.list-workspace.active{height:100%;min-height:0;overflow:hidden;}
            .pool-table-wrap{height:auto;max-height:calc(100vh - 355px);min-height:0;overflow:auto;}
            .list-editor-form{height:100%;min-height:0;}
            .list-editor-form textarea{min-height:0;}
        }
        @media (min-width: 761px){
            ...
            .app-caption strong,.app-branch,.section-subtitle{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;overflow-wrap:normal;word-break:normal;}
            .workspace-layout{flex:1;min-height:0;gap:8px;align-items:stretch;}
            .app-main{height:100%;min-height:0;overflow:hidden;}
            .app-view.active{height:100%;min-height:0;overflow:hidden;}
            .app-view[data-view="status"].active{display:grid;grid-template-rows:auto auto auto auto;gap:8px;align-content:start;}
            .app-view[data-view="keys"].active,.app-view[data-view="lists"].active{display:grid;grid-template-rows:auto auto minmax(0,1fr);gap:8px;}
            .view-head,.segmented,.status-dashboard,.overview-service-grid{margin-bottom:0;}
            .status-dashboard{min-height:0;overflow:hidden;}
            .status-dashboard-column{min-height:0;}
            .status-card{min-height:0;overflow:hidden;}
            .router-health-card #router-health-note{overflow:auto;}
            .protocol-panels,.list-panels{min-height:0;overflow:hidden;}
            .protocol-workspace.active{height:100%;min-height:0;display:grid;grid-template-rows:auto auto minmax(0,1fr);overflow:hidden;}
            .protocol-subview.active{min-height:0;overflow:hidden;}
            .protocol-subview[data-subview="pool"].active{display:grid;grid-template-rows:auto auto minmax(0,1fr);overflow:hidden;}
            .pool-table-wrap{min-height:0;overflow:auto;}
            .list-workspace.active{height:100%;min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr);overflow:hidden;}
            .list-editor-form{height:100%;min-height:0;display:grid;grid-template-rows:minmax(0,1fr) auto auto;overflow:hidden;}
            .list-editor-form textarea{height:100%;min-height:0;resize:none;}
        }
        ...
        @media (min-width: 761px){
            ...
            .app-view[data-view="status"].active{grid-template-rows:auto auto auto auto;align-content:start;gap:8px;overflow:hidden;}
            .status-dashboard{overflow:visible;align-items:stretch;margin-bottom:0;}
            .status-dashboard-column{min-height:auto;}
            .status-dashboard-with-pool{align-items:stretch;}
            .status-dashboard-with-pool .status-dashboard-column{display:grid;grid-template-rows:auto minmax(0,1fr);gap:8px;align-content:stretch;align-items:stretch;min-width:0;height:100%;}
            .status-dashboard-with-pool .status-dashboard-column-primary .quick-start-card{align-self:start;}
            .status-dashboard-with-pool .router-health-card,.status-dashboard-with-pool .key-pool-card{height:100%;align-self:stretch;}
            ...
            .router-health-actions{margin-top:6px;}
            ...
            .protocol-subview-key .key-editor-form .form-actions{grid-column:2;grid-row:2;margin:0;display:grid;align-items:start;}
            .protocol-subview-key .key-editor-form .form-actions button{width:100%;min-width:0;height:var(--control-height);min-height:var(--control-height);padding:0 10px;}
            .protocol-subview-key .pool-import-form textarea{height:112px;min-height:112px;max-height:none;resize:none;overflow:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;}
        ...
        @media (min-width: 1024px){
            ...
            .status-overview-copy{flex-wrap:nowrap;}
            ...
            .app-view[data-view="status"].active .overview-key-panel{display:grid;align-self:stretch;height:100%;min-height:0;}
            .app-view[data-view="status"].active .overview-key-panel .key-editor-form{height:100%;min-height:0;grid-template-rows:auto minmax(44px,1fr);}
            .app-view[data-view="status"].active .overview-key-panel textarea{height:100%;min-height:44px;max-height:none;}
        }
        @media (min-width: 761px) and (max-width: 1023px){
            .app-view[data-view="status"].active{overflow:auto;padding-right:2px;}
            .status-dashboard-with-pool{grid-template-columns:1fr;}
            .status-dashboard-column{min-height:auto;}
            .status-dashboard-with-pool .status-dashboard-column{display:grid;min-height:auto;grid-template-rows:auto auto;}
        ...
        .confirm-backdrop{position:fixed;inset:0;z-index:700;display:flex;align-items:center;justify-content:center;padding:18px;background:rgba(2,6,23,.72);}
        ...
        @media (max-width: 760px){
            ...
            .topbar{position:relative;z-index:260;top:0;align-items:stretch;flex-direction:column;margin-bottom:var(--mobile-status-gap);padding:10px;}
            ...
            .topbar-actions.topbar-actions-web-only{grid-template-columns:repeat(2,minmax(0,1fr));}
            ...
            .app-caption strong,.app-branch{max-width:100%;min-width:0;overflow-wrap:anywhere;word-break:break-word;}
            .app-caption strong{max-width:none;padding-right:60px;font-size:14px;line-height:1.18;white-space:normal;}
            ...
            .mode-control #mode-picker{left:0;right:auto;}
            .theme-control .theme-picker{right:0;left:auto;}
            ...
            .version-badge{grid-column:2;grid-row:1;justify-self:end;align-self:start;width:auto;min-width:48px;min-height:22px;padding:3px 7px;font-size:10px;}
            ...
            .app-view[data-view="status"].active{display:grid;grid-template-columns:1fr;gap:var(--mobile-status-gap);align-content:start;}
            .app-view[data-view="status"].active > .view-head,
            .app-view[data-view="status"].active > .status-dashboard,
            .app-view[data-view="status"].active > .overview-service-grid,
            .app-view[data-view="status"].active > .overview-key-panel{margin-top:0;margin-bottom:0;}
            .mobile-nav{position:fixed;left:10px;right:10px;bottom:calc(10px + env(safe-area-inset-bottom, 0px));z-index:240;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:4px;padding:6px;border:1px solid rgba(91,124,150,.34);border-radius:var(--radius-panel);background:rgba(12,18,26,.96);box-shadow:0 14px 34px rgba(0,0,0,.34);}
            ...
            .event-history-backdrop{z-index:720;align-items:stretch;justify-content:stretch;padding:0;height:100dvh;overflow:hidden;}
            .event-history-drawer{width:100%;height:100dvh;max-height:100dvh;border-radius:0;border-left:0;border-right:0;padding:12px;}
            .event-history-drawer-head{position:sticky;top:0;z-index:1;padding-bottom:8px;background:rgba(17,25,35,.96);}
            .event-history-drawer .event-history-list{padding-bottom:calc(96px + env(safe-area-inset-bottom, 0px));}
            .view-head{padding:14px;border-radius:10px;}
            .view-head h2{font-size:20px;}
            .status-overview-head{gap:8px;padding:12px;}
            .status-overview-copy{display:block;}
            .status-head-attention{grid-template-columns:1fr;}
            .status-dashboard,.overview-service-grid{grid-template-columns:1fr;gap:var(--mobile-status-gap);}
            .status-dashboard-column{display:grid;gap:var(--mobile-status-gap);}
            .overview-service-grid{gap:var(--mobile-status-gap);margin-top:0;}
            .overview-key-panel{margin-top:0;}
            ...
            .protocol-subview-key .key-editor-form textarea[data-key-textarea]{height:156px;min-height:156px;max-height:none;resize:none;white-space:pre-wrap;overflow:hidden;overflow-wrap:anywhere;word-break:break-all;}
            ...
            .protocol-subview-key.active{gap:8px;overflow:visible;}
            .protocol-subview-key .key-editor-form,.protocol-subview-key .pool-import-form{padding:8px;}
            .pool-import-form{grid-template-columns:1fr;}
            .pool-import-form textarea{height:132px;min-height:132px;max-height:none;resize:none;overflow:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;}
            .pool-import-form button{grid-column:1 / -1;justify-self:stretch;width:100%;}
            ...
            .pool-controls{grid-template-columns:1fr;gap:6px;margin-bottom:8px;}
            ...
            .pool-apply-btn{display:flex;width:100%;font-size:10.5px;line-height:1.18;text-align:left;justify-content:flex-start;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;gap:4px;}
            ...
            .pool-actions-cell .pool-delete-btn::before{content:none;}
            ...
            .hero-popover{min-width:0;width:100%;}
            .app-mode-control .app-mode-picker{position:static;min-width:0;width:100%;margin-top:8px;}
            .topbar .hero-popover{position:absolute;width:min(260px,calc(100vw - 42px));z-index:330;}
        ...
        @media (max-width: 430px){
            ...
            .app-caption{grid-column:1 / -1;grid-row:1;}
            .version-badge{grid-column:2;grid-row:1;}
            .api-pill{grid-column:1 / -1;grid-row:2;}
            .command-grid{grid-template-columns:1fr;}
            .command-grid.service-command-grid{grid-template-columns:1fr;}
        ...
        [data-theme="light"]{
            ...
            --scrollbar-track:#dde4eb;
            --scrollbar-thumb:#9fb2c3;
            --scrollbar-thumb-hover:#7f94a9;
            --focus-ring:0 0 0 3px rgba(31,122,106,.14);
        }
        [data-theme="light"] body{color:var(--text);background:linear-gradient(145deg,#e6ebf0 0%,#dce3ea 48%,#d4dce5 100%);}
        ...
        [data-theme="light"] .status-card{background:rgba(231,235,240,.94);border-color:var(--border);box-shadow:var(--shadow);backdrop-filter:none;}
        [data-theme="light"] .service-panel,
        [data-theme="light"] .overview-key-panel,
        [data-theme="light"] .status-card{background:linear-gradient(145deg,rgba(231,235,240,.96),rgba(216,223,232,.96));}
        [data-theme="light"] h1,
        [data-theme="light"] h2,
        [data-theme="light"] h3,
        ...
        [data-theme="light"] .traffic-chip{background:rgba(225,230,236,.9);border-color:var(--border);color:var(--text);}
        [data-theme="light"] .api-pill{color:#1f564f;}
        [data-theme="light"] .topbar-status-copy strong{color:#1f2933;}
        [data-theme="light"] .topbar-status-copy span{color:#5d6a78;}
        [data-theme="light"] .topbar-status-icon{background-color:rgba(31,122,106,.12);border-color:rgba(31,122,106,.24);}
        [data-theme="light"] .topbar-status-ok{background:rgba(31,122,106,.08);border-color:rgba(31,122,106,.24);}
        [data-theme="light"] .topbar-status-info{background:rgba(225,230,236,.9);border-color:var(--border);}
        [data-theme="light"] .topbar-status-warn{background:rgba(220,226,233,.9);border-color:rgba(115,128,145,.34);}
        [data-theme="light"] .topbar-status-danger{background:rgba(241,214,208,.72);border-color:rgba(168,68,47,.3);}
        ...
        [data-theme="light"] .hero-popover{background:linear-gradient(180deg,rgba(229,234,240,.98),rgba(216,223,232,.96));border-color:var(--border);}
        ...
        [data-theme="light"] .service-route-menu-list{background:var(--surface);border-color:var(--border);box-shadow:0 16px 34px rgba(43,55,71,.14);}
        ...
        [data-theme="light"] .pool-sort-button,
        [data-theme="light"] .pool-sort-menu{background:var(--surface);border-color:var(--border);color:var(--text);}
        ...
        [data-theme="light"] .protocol-subview-key .key-editor-form,
        [data-theme="light"] .protocol-subview-key .pool-import-form,
        ...
        [data-theme="light"] .event-history-item{background:rgba(225,230,236,.86);border-color:var(--border);}
        ...
        [data-theme="light"] .tab-count{background:#d9e1ea;color:#233144;}
        [data-theme="light"] .pool-table th{background:#e1e7ee;color:#536274;}
        [data-theme="light"] .pool-sort-option{color:var(--text);}
        [data-theme="light"] .pool-sort-option:hover,
        [data-theme="light"] .pool-sort-option.active{background:rgba(31,122,106,.1);border-color:rgba(31,122,106,.24);color:#1f6258;}
        [data-theme="light"] .pool-table th,
        [data-theme="light"] .pool-table td{border-bottom-color:#cad3dd;}
        ...
        [data-theme="light"] .key-status-warn{background:#e9eef5;border-color:#bdc8d6;color:#536274;}
        ...
        [data-theme="light"] .service-probe-unknown{background:#e3eaf1;border-color:var(--border);color:#536274;}
        [data-theme="light"] .service-probe-fail{background:#f7dedb;border-color:#d79891;color:#92352d;}
        [data-theme="light"] .service-probe-na{background:transparent;border-color:#d7dee8;color:#7b8796;}
        [data-theme="light"] .custom-check-item{background:rgba(222,228,235,.86);border-color:#c0cad5;}
        [data-theme="light"] .custom-check-empty{border-color:var(--border);background:rgba(218,225,233,.7);}
        [data-theme="light"] .notice-result{background:var(--warn-bg);border-color:var(--warn-border);color:#415064;}
        ...
        [data-theme="glass"]{
            ...
            --surface:#101b24;
            --surface-soft:#13222d;
            --surface-strong:#1b2e3a;
            --border:rgba(151,226,222,.2);
            --text:#f2f8fb;
            --muted:#aabaca;
            --primary:#9ceee7;
            --primary-hover:#c4fff9;
            --secondary:#e6c47e;
            --danger:#dd786f;
            --success-bg:rgba(46,129,97,.18);
            --success-border:rgba(134,230,184,.28);
            --warn-bg:rgba(139,103,42,.17);
            --warn-border:rgba(226,190,118,.32);
            --shadow:0 16px 42px rgba(0,0,0,.34);
            --scrollbar-track:#071018;
            --scrollbar-thumb:#3f5662;
            --scrollbar-thumb-hover:#5d7883;
            --focus-ring:0 0 0 3px rgba(156,238,231,.16);
            --glass-bg:rgba(18,30,39,.54);
            --glass-bg-strong:rgba(24,42,52,.68);
            --glass-border:rgba(167,238,232,.23);
            --glass-highlight:rgba(255,255,255,.18);
            --glass-shadow:0 12px 30px rgba(0,0,0,.26), inset 0 1px 0 rgba(255,255,255,.12);
            --glass-blur:6px;
        }
        [data-theme="glass"] body{
            color:var(--text);
            background:
                radial-gradient(circle at 12% -10%, rgba(229,196,126,.12), transparent 34%),
                radial-gradient(circle at 82% 0%, rgba(156,238,231,.12), transparent 36%),
                linear-gradient(135deg, rgba(8,15,22,.99), rgba(12,25,33,.98) 46%, rgba(8,13,20,.99));
        }
        [data-theme="glass"] body::before{
            ...
            pointer-events:none;
            background:
                linear-gradient(120deg, rgba(255,255,255,.045), transparent 31%, rgba(156,238,231,.025) 58%, transparent 82%),
                linear-gradient(180deg, rgba(255,255,255,.025), transparent 45%, rgba(229,196,126,.018));
            opacity:.58;
        }
        ...
        [data-theme="glass"] .subtabs{
            background:
                linear-gradient(145deg, rgba(255,255,255,.105), rgba(255,255,255,.028) 44%, rgba(156,238,231,.055)),
                var(--glass-bg);
        ...
        [data-theme="glass"] .status-card{
            background:
                linear-gradient(145deg, rgba(20,33,42,.9), rgba(10,18,27,.86)),
                rgba(10,18,27,.86);
            border-color:rgba(167,238,232,.14);
        ...
        [data-theme="glass"] .pool-sort-button,
        [data-theme="glass"] .mode-choice,
        [data-theme="glass"] .seg-tab,
        [data-theme="glass"] .subtab,
        [data-theme="glass"] .nav-item{
            background:
                linear-gradient(145deg, rgba(255,255,255,.085), rgba(255,255,255,.024) 52%, rgba(156,238,231,.045)),
                rgba(17,35,43,.44);
            border-color:rgba(167,238,232,.22);
            color:#b8f5ef;
            box-shadow:inset 0 1px 0 rgba(255,255,255,.09);
            backdrop-filter:none;
            -webkit-backdrop-filter:none;
            transition:transform .16s cubic-bezier(.2,.8,.2,1), background-color .16s ease, border-color .16s ease, box-shadow .16s ease, color .16s ease;
        }
        @media (hover: hover){
            ...
            [data-theme="glass"] .nav-item:hover{
                ...
                border-color:rgba(190,255,249,.42);
                background:
                    linear-gradient(145deg, rgba(255,255,255,.12), rgba(255,255,255,.032) 48%, rgba(156,238,231,.075)),
                    rgba(22,48,56,.52);
                box-shadow:0 8px 20px rgba(0,0,0,.16), inset 0 1px 0 rgba(255,255,255,.16);
            }
        }
        [data-theme="glass"] [data-liquid]:focus-visible,
        [data-theme="glass"] [data-liquid].liquid-active{
            background:
                radial-gradient(circle at var(--mx, 50%) var(--my, 50%), rgba(255,255,255,.16), rgba(156,238,231,.075) 24%, transparent 58%),
                linear-gradient(145deg, rgba(255,255,255,.13), rgba(255,255,255,.035) 48%, rgba(156,238,231,.08)),
                rgba(22,48,56,.52);
            border-color:rgba(196,255,249,.44);
            box-shadow:0 8px 20px rgba(0,0,0,.16), inset 0 1px 0 rgba(255,255,255,.18), inset 0 -1px 0 rgba(156,238,231,.08);
        }
        @media (hover: hover){
        ...
        [data-theme="glass"] button.danger{
            background:linear-gradient(145deg, rgba(255,205,197,.1), rgba(88,31,37,.5));
            border-color:rgba(239,132,121,.38);
            color:#ffd0ca;
        }
        @media (hover: hover){
            [data-theme="glass"] button.danger:hover{
                background:linear-gradient(145deg, rgba(255,219,212,.14), rgba(113,42,48,.6));
                border-color:rgba(255,162,152,.52);
            }
        }
        [data-theme="glass"] .mode-choice.active,
        [data-theme="glass"] .seg-tab.active,
        ...
        [data-theme="glass"] [data-theme-choice].active{
            background:
                linear-gradient(145deg, rgba(255,255,255,.16), rgba(156,238,231,.14) 56%, rgba(230,196,126,.09)),
                rgba(28,65,70,.58);
            border-color:rgba(188,255,249,.44);
            color:#f7fffe;
            box-shadow:0 8px 22px rgba(32,211,198,.08), inset 0 1px 0 rgba(255,255,255,.18), inset 0 -2px 0 rgba(156,238,231,.24);
        }
        [data-theme="glass"] .mobile-nav .nav-item.active{
            background:
                linear-gradient(145deg, rgba(255,255,255,.08), rgba(156,238,231,.05)),
                rgba(17,35,43,.44);
            border-color:rgba(167,238,232,.3);
            color:#e7fffd;
            box-shadow:inset 0 -3px 0 rgba(156,238,231,.48), inset 0 1px 0 rgba(255,255,255,.12);
        }
        [data-theme="glass"] input,
        [data-theme="glass"] textarea,
        [data-theme="glass"] select,
        [data-theme="glass"] .pool-sort-menu{
            background:rgba(6,13,20,.6);
            border-color:rgba(167,238,232,.18);
            color:var(--text);
        ...
        [data-theme="glass"] .pool-table th{background:rgba(14,28,37,.78);color:#b7cad8;}
        [data-theme="glass"] .pool-sort-menu{background:linear-gradient(145deg,rgba(20,33,42,.98),rgba(10,18,27,.96));box-shadow:0 18px 46px rgba(0,0,0,.46), inset 0 1px 0 rgba(255,255,255,.08);}
        [data-theme="glass"] .pool-sort-option{color:#d8f4f1;}
        [data-theme="glass"] .pool-sort-option:hover,
        [data-theme="glass"] .pool-sort-option.active{background:rgba(156,238,231,.11);border-color:rgba(167,238,232,.24);color:#9ffff7;}
        [data-theme="glass"] .pool-row-active{background:rgba(156,238,231,.065);}
        ...
        [data-theme="glass"] .mobile-nav[data-liquid],
        [data-theme="glass"] .side-nav[data-liquid],
        [data-theme="glass"] .segmented[data-liquid],
        [data-theme="glass"] .subtabs[data-liquid]{
            overflow:hidden;
        }
        ...
        [data-theme="glass"] [data-liquid]::before{
            ...
            background:
                radial-gradient(circle at var(--mx, 50%) var(--my, 50%), rgba(255,255,255,.13), rgba(156,238,231,.045) 24%, transparent 56%),
                linear-gradient(115deg, transparent 0 30%, rgba(255,255,255,.07) 47%, transparent 66% 100%);
            opacity:0;
        ...
        [data-theme="glass"] [data-liquid]::after{
            ...
            background:
                radial-gradient(circle at 32% 24%, rgba(255,255,255,.12) 0 4px, rgba(255,255,255,.04) 18%, transparent 45%),
                conic-gradient(from 30deg, transparent 0 18%, rgba(255,255,255,.075) 21%, transparent 34% 61%, rgba(156,238,231,.06) 66%, transparent 78%);
            opacity:0;
        ...
        [data-theme="glass"] [data-liquid].liquid-active::after{opacity:.6;}
        ...
        @media (hover: none), (pointer: coarse){
            ...
            [data-theme="glass"] [data-liquid].liquid-active::before{opacity:.32;}
            [data-theme="glass"] [data-liquid].liquid-active::after{opacity:0;}
            [data-theme="glass"] [data-liquid-group="true"].liquid-active::before{opacity:.4;}
        ...
        .liquid-global-lens{
            ...
            border:1px solid rgba(236,255,253,.22);
            box-shadow:
                inset 0 1px 2px rgba(255,255,255,.32),
                inset 0 -12px 20px rgba(6,18,26,.09),
                0 3px 12px rgba(0,0,0,.08),
                0 0 16px rgba(156,238,231,.045);
        ...
        .liquid-global-lens::before{
            background:
                radial-gradient(circle at 20% 18%, rgba(255,255,255,.3) 0 6px, rgba(255,255,255,.085) 26%, rgba(255,255,255,.018) 46%, transparent 62%),
                radial-gradient(circle at 72% 74%, rgba(156,238,231,.022), transparent 66%),
                linear-gradient(130deg, rgba(255,255,255,.13), transparent 39%, rgba(255,255,255,.045) 68%, transparent 100%);
        }
        .liquid-global-lens::after{
            inset:9px;
            border:1px solid rgba(255,255,255,.17);
            box-shadow:inset 10px 8px 15px rgba(255,255,255,.065), inset -14px -16px 24px rgba(6,18,26,.085);
            opacity:.66;
        }
