...
- Hysteria2 поддерживает горячее переключение между серверами и возврат к прежним параметрам, включая Salamander. В официальном Xray 26.2.6 кеш клиента не учитывает новый пароль и TLS-параметры на уже использованном адресе; программа выявляет такие коллизии и сохраняет для них применение с перезапуском. Для иных транспортов, расширенных настроек Hysteria2, первого добавления протокола и изменения структуры маршрутов также сохраняется прежний способ применения. Ручной выбор и автоматика используют общий механизм; ошибка горячего применения сама по себе никогда не запускает общий перезапуск.
