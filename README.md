...
- Смена ключей Vless 1, Vless 2 и Vmess без общего перезапуска на проверенной сборке Xray 26.2.6: для поддержанных TCP/raw, TLS, REALITY, Vision и mux-сочетаний старые соединения сохраняются, новые используют проверенный ключ. Неизменный ключ подтверждается по загруженному состоянию и передаче данных. Для других транспортов и протоколов с отдельными сервисами, а также при изменении маршрутов, сохраняется применение с перезапуском.
