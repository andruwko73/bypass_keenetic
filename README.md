...
- **Пул ключей** — возможность хранить несколько ключей для каждого протокола (Vless 1, Vless 2, Vmess, Trojan, Shadowsocks), применять нужный ключ кликом из пула и очищать весь пул выбранного протокола одной кнопкой. В Telegram-боте пул доступен из меню **Ключи и мосты** и работает через inline-кнопки: применить ключ, удалить конкретный ключ, загрузить subscription, проверить пул и вернуться назад.
...
- список ключей отображается страницами, под каждым ключом есть inline-кнопки **✅ номер** для применения и **✕ номер** для удаления конкретного ключа;
...
Пул ключей в Telegram-боте в тёмной теме с inline-кнопками применения, удаления, проверки и навигации:
