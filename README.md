...
- Смена ключей всех шести пулов без общего перезапуска на проверенной сборке Xray 26.2.6 — в пределах проверенных вариантов транспорта. Vless 1/2 и Vmess поддерживают TCP/raw, TLS, REALITY, Vision и допустимые mux-сочетания; Trojan — TCP/raw с TLS или без него, Shadowsocks — штатный TCP/raw. Для Trojan и Shadowsocks перезапускается только собственная служба прозрачного прокси: её соединения могут прерваться, другие протоколы и общий Xray продолжают работу. При ошибке восстанавливаются прежние файлы, служба и маршрут.
