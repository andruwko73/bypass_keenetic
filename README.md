...
Если общий Google-IP уже оказался в приоритетном маршруте YouTube/Vless 2, Xray дополнительно распознаёт TCP-домен Chrome Remote Desktop и возвращает его в единственный маршрут, полностью содержащий адреса сервиса. Правило не создаётся при неоднозначном назначении и не изменяет пользовательские списки, ipset или маршрутизацию звонков.
