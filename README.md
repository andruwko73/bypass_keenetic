...
Начиная с v1.801 внешний prefetch сначала проверяет YouTube edge-IP через активный маршрут и добавляет только быстрые кандидаты, отбрасывая медленные, TLS EOF и timeout-ответы. В v1.802 fast-warm при установке и обновлении сначала берёт свежие bootstrap DNS-кандидаты, а не старый cache, и измеряет уже присутствующие YouTube IP без вывода этих внутренних данных в веб-интерфейс. В v1.804 fast-warm дополнительно прогревает `youtube.com`, `i.ytimg.com`, `s.ytimg.com`, `yt3.ggpht.com` и второй live watch URL, потому что именно эти запросы чаще всего задерживают главную страницу и live-видео. В v1.805 перед тяжёлым прогревом короткий внешний restore возвращает в активный ipset последние YouTube edge-IP, которые уже проходили quality-проверку; это уменьшает ожидание после обновления или рестарта и не добавляет постоянной работы в процесс Telegram-бота. Общие Google/Chrome Remote Desktop хосты не добавляются в YouTube runtime ipset, чтобы ускорение YouTube не ломало CRD/auth-трафик.
