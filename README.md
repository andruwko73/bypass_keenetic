...
- пул ключей, сервисные списки и обновление из ветки independent прямо из веб-интерфейса и Telegram-бота
...
- **Пул ключей** — возможность хранить несколько ключей для каждого протокола (Vless 1, Vless 2, Vmess, Trojan, Shadowsocks), применять нужный ключ кликом из пула и очищать весь пул выбранного протокола одной кнопкой. В Telegram-боте пул доступен из меню **Ключи и мосты** и управляется нижними кнопками: применить ключ, удалить конкретный ключ, загрузить subscription, проверить пул и вернуться назад.
...
- **Subscription-ссылки** — поддержка загрузки ключей из subscription-URL (подписки) прямо через веб-интерфейс и Telegram-бота.
...
- **Переход в web-only** — отдельная кнопка в веб-интерфейсе переустанавливает версию из ветки `feature/web-only` без Telegram-бота, сохраняя локальные ключи и списки.
...
- фоновые команды обновления и переустановки с прогрессом выполнения на странице;
- переключение активного режима для самого Telegram-бота;
- редактирование списков обхода по протоколам;
- служебные команды: обновление из форка без сброса, переустановка из ветки independent, переход в версию без Telegram-бота, перезапуск сервисов, DNS override, удаление компонентов и перезагрузка.
...
6. Используйте **Пул ключей** для хранения нескольких ключей: выбранный ключ можно применить кликом по кнопке с его названием, отдельный ключ удалить через режим **Удаление**, новые ключи загрузить вручную или через subscription, а весь пул очистить кнопкой **Очистить пул**.
...
В ветке `feature/independent-rework` Telegram-бот умеет управлять тем же пулом ключей, который отображается в веб-интерфейсе.

Путь в меню: **🔑 Ключи и мосты** → **📦 Пул ключей**.

Внутри пула:
- сначала выбирается протокол: **Vless 1**, **Vless 2**, **Vmess**, **Trojan** или **Shadowsocks**;
- все ключи выбранного протокола выводятся в прокручиваемой нижней клавиатуре с названием, номером и статусами **TG** Telegram / **YT** YouTube; нажатие на кнопку ключа применяет его;
- каждая кнопка ключа содержит короткий код протокола: **V1**, **V2**, **VM**, **TR** или **SS**, поэтому нажатие старой кнопки из другого пула не применит ключ к неверному протоколу;
- служебные кнопки **Добавить**, **Проверить**, **Удаление** и навигация идут после списка ключей в той же прокручиваемой клавиатуре;
- кнопка **🗑 Удаление** переключает нижнюю клавиатуру в режим удаления конкретного ключа из пула;
- кнопка **➕ Добавить** переводит бота в режим вставки одного или нескольких ключей, по одному ключу на строку;
- кнопка **🔗 Subscription** загружает ключи из subscription-URL;
- кнопка **🔍 Проверить** запускает фоновую проверку пула, статусы Telegram/YouTube обновляются после проверки;
- кнопка **🧹 Очистить** очищает весь пул выбранного протокола только после подтверждения;
- кнопки **🔙 К протоколам** и **В меню ключей** возвращают пользователя назад без сброса сохранённых ключей.

Применение ключа из Telegram-бота запускается в фоне: бот сразу отвечает, что ключ применяется, а после перезапуска proxy/Xray и проверки связи обновляет сообщение с итогом. Это нужно, чтобы Telegram-кнопка не выглядела зависшей во время технического рестарта сервисов.

### Скриншоты интерфейса

Пул ключей в Telegram-боте в тёмной теме: сообщение остаётся текстовым, а применение, удаление, проверка и навигация доступны нижними кнопками:
...
Команды установки и обслуживания, включая переустановку в web-only, и списки обхода:
