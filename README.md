...
Начиная с v1.801 внешний prefetch сначала проверяет YouTube edge-IP через активный маршрут и добавляет только быстрые кандидаты, отбрасывая медленные, TLS EOF и timeout-ответы. В v1.802 fast-warm при установке и обновлении сначала берёт свежие bootstrap DNS-кандидаты, а не старый cache, и измеряет уже присутствующие YouTube IP без вывода этих внутренних данных в веб-интерфейс. Общие Google/Chrome Remote Desktop хосты не добавляются в YouTube runtime ipset, чтобы ускорение YouTube не ломало CRD/auth-трафик.
