...
Для двух `Vless`-маршрутов UDP/443 обрабатывается умно: основные наборы `unblockvless` и `unblockvless2` отвечают за TCP и обычную UDP-маршрутизацию через прозрачный прокси, а `unblockvlessudp` и `unblockvless2udp` наполняются только YouTube/QUIC-доменами из соответствующего списка. Такой UDP/QUIC-трафик быстро отклоняется на прозрачном порту, чтобы браузер без долгого ожидания переходил на TCP через выбранный Vless. Поэтому YouTube может лежать в любом из двух Vless-списков, а Telegram-домены не получают широкую UDP-блокировку и сохраняют обычный UDP-путь для медиа и звонков. TCP-порты мобильных push-сервисов Android/iOS (`5223`, `5228`, `5229`, `5230`) автоматически направляются в тот Vless-маршрут, где находится Telegram. При необходимости QUIC-блокировку можно отключить в `bot_config.py` флагами `udp_quic_block_vless_enabled` и `udp_quic_block_vless2_enabled`.
