...
- DNS-правила списков формируются одним пакетным проходом; неизменная конфигурация не требует перезапуска DNS. При ошибке сохраняются прежние рабочие правила. Кэш проверки хранится в памяти роутера.

- Веб-интерфейс на `http://192.168.1.1:8080/` для ПК и телефона.
- Разделы **Статус**, **Ключи**, **Списки**.
- Компактная шапка «Панель управления»; карточки состояния и сервисные действия доступны сразу под ней.
...
- Поддержка двух режимов `Vless`, а также протоколов `Vmess`, `Trojan`, `Hysteria2`, `Shadowsocks`.
- Пул ключей для каждого протокола: ручное добавление, удаление, применение, импорт нескольких подписок.
- Смена ключей всех шести пулов без общего перезапуска на проверенной сборке Xray 26.2.6 — в пределах проверенных вариантов транспорта. Vless 1/2 и Vmess поддерживают TCP/raw, TLS, REALITY, Vision и допустимые mux-сочетания; Trojan — TCP/raw с TLS, Shadowsocks — штатный TCP/raw. Для Trojan и Shadowsocks перезапускается только собственная служба прозрачного прокси: её соединения могут прерваться, другие протоколы и общий Xray продолжают работу. При ошибке восстанавливаются прежние файлы, служба и маршрут.
- Hysteria2 поддерживает горячее переключение между серверами и возврат к прежним параметрам, включая Salamander. В официальном Xray 26.2.6 кеш клиента не учитывает новый пароль и TLS-параметры на уже использованном адресе; программа выявляет такие коллизии и применяет их с контролируемым перезапуском. Перезапуск также может потребоваться для иных транспортов и изменения структуры маршрутов. Ошибка горячего применения сама по себе никогда не запускает общий перезапуск.
- Ручное применение и удаление работают независимо от доступности сервера: проверяются формат ключа, локальная конфигурация и сохранение настроек; результат проверки связи появляется отдельно. Недоступная замена не мешает удалить активный ключ. Удаление последнего ключа отключает направление, сохраняя его списки; трафик не переводится напрямую. На Xray 26.2.6 первое добавление и последнее удаление используют заранее подготовленные выходы. Автоматическое аварийное переключение по-прежнему требует подтверждения работоспособности кандидата.
- При переносе списков DNS-запросы объединяются между протоколами и приоритетными адресами. Ограниченный кэш хранится в RAM с учётом DNS TTL; тайм-аут не считается отрицательным ответом. После ошибки допустимо кратко использовать прежний положительный ответ без продления срока его хранения. Обновляются затронутые наборы и их зависимости; при утрате состояния выполняется полное обновление. Неизменившаяся подтверждённая конфигурация Xray не перезапускается. Панель показывает текущий этап переноса.
- Проверка ключей через Telegram API, YouTube и дополнительные сервисы: ChatGPT/OpenAI/Codex, Claude, Gemini, Copilot, Perplexity, Grok/X/Twitter, DeepSeek, Discord, Chrome Remote Desktop, Instagram/Facebook и TikTok.
- Автоматическое переключение Telegram и YouTube проверяет также включённые дополнительные сервисы заменяемого маршрута — до применения ключа и через рабочий прокси после него. Свежий отказ обязательного сервиса исключает кандидат на пять минут, устаревший перепроверяется. Если подходящей замены нет, прежний ключ сохраняется; причины видны в истории событий. Ручной выбор остаётся доступен. Произвольные адреса без настроенной проверки не считаются проверенными сервисами.
- Прогрев YouTube для ускорения загрузки видео.
- Защита просмотра YouTube от ложной смены ключа: при продолжающемся медиатрафике одиночная нестабильная ошибка контрольного URL не запускает аварийное переключение. Полный отказ подтверждается несколькими YouTube-точками; жёсткие ошибки прокси сохраняют аварийную обработку. После отклонения кандидатов исходный ключ восстанавливается и проверяется заново, а повторная аварийная попытка откладывается на пять минут по умолчанию.
...
- Страница **Диагностика маршрутов**: сохранённые профили устройства и назначения, проверка до трёх активных прокси через выбранное интернет-подключение. Проверка запускается вручную, подтверждает фактический выход и сравнивает время HTTPS-ответов. Результат не меняет ключи и маршруты; задержка игры и потери UDP без отдельного измерения показываются как «нет данных». Измерение выполняется с роутера, поэтому не включает Wi‑Fi устройства.
- Состояние роутера обновляется одной фоновой задачей: медленная диагностика не задерживает открытие страницы. При обновлении показан возраст предыдущего результата, при ошибке — сообщение о повторной попытке.
- Открытие панели и просмотр пулов не ждут окончания применения списков и ключей. Активный ключ включается в отображаемый пул без записи файлов при чтении страницы.
- Списки IP-адресов и доменов разбираются пакетно, без отдельного запуска shell-команд для каждой строки; расписание и правила маршрутизации сохраняются.
- После успешного обновления установочные архивы очищаются автоматически; сохраняется одна последняя полная копия программы и настроек для отката.
...
После первого успешного обновления полная копия обновления заменяет эту первоначальную копию. Для отката используется действие в панели или `/opt/root/bypass-last-update-rollback.sh`; временные каталоги загрузки удаляются после проверки запуска служб.
...
Устаревшие артефакты старых вариантов очищаются: отдельный `web_bot.py`, старый web-only init-скрипт, tor/vpn пути и лишние runtime-файлы. Остаются только поддерживаемые протоколы: два `Vless`, `Vmess`, `Trojan`, `Hysteria2`, `Shadowsocks`.
...
Для мобильных уведомлений Telegram TCP/5222 следует за тем протоколом, в списке которого находится Telegram: Shadowsocks, VMess, VLESS1, VLESS2, Trojan или Hysteria2. Системные каналы push для iOS APNs (TCP/5223) и Android FCM (TCP/5228-5230) проходят напрямую, а слишком короткий таймаут установленных conntrack-соединений автоматически поднимается до 3600 секунд.
...
Сообщения о ходе установки, обновления и отката выводятся на русском языке. Технические имена Xray, GitHub, SOCKS, API, пути и коды ошибок сохраняются в исходном виде, чтобы диагностика оставалась точной.
...
Ключи VLESS в обоих пулах сохраняют TLS-параметры `pcs` и `vcn` для проверки закреплённого сертификата, отпечаток клиента `fp` и список `alpn`. Для XHTTP сохраняются `path`, `host`, `mode` и JSON-параметр `extra`; прежнее имя транспорта `splithttp` также принимается.

Ключи Hysteria2 принимаются со схемами `hysteria2` и `hy2`, включая ссылку подписки. Поддерживаются auth, SNI, ALPN, SHA-256 pin сертификата, обфускация Salamander и JSON-параметр `fm`. Для `insecure=1` нужен `pinSHA256`: проверка сертификата не отключается. Параметры `fm.quicParams` переводятся в схему установленного ядра: до Xray 26.3.27 — в `hysteriaSettings`, начиная с этой версии — в `finalmask.quicParams`. Совместимые настройки смены UDP-порта можно передать через `fm.quicParams.udpHop`; несколько портов непосредственно в адресе URI и ECH по-прежнему отклоняются. Неизвестные старому ядру QUIC-параметры и одновременное задание `obfs` с непустым `fm.udp` вызывают явную ошибку вместо потери настроек. Наличие ключа в правильном пуле не гарантирует доступность его сервера или всех контрольных адресов.
