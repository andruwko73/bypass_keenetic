...
Ключи Hysteria2 принимаются со схемами `hysteria2` и `hy2`, включая ссылку подписки. Поддерживаются стандартные auth, SNI, ALPN, `insecure` и SHA-256 pin сертификата. Port hopping, ECH и obfs отклоняются с явной ошибкой: текущий Xray на поддерживаемых роутерах не обеспечивает для них надёжную совместимость, поэтому программа не сохраняет такой ключ как будто он рабочий.
