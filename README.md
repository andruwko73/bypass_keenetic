...
Для `Vless 2` используется два набора: `unblockvless2` для TCP-маршрутизации через прозрачный прокси и `unblockvless2udp` для точечной блокировки QUIC/UDP 443 по доменным IP. Это помогает YouTube уходить через `Vless 2`, не закрывая лишние Google-сети для других сервисов.
